...
folder are mapped to the application's root namespace, 'OrderMaint'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "OrderMaint"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'OrderMaint' in the same way
as `"OrderMaint/sass/src"`.
